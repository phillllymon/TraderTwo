...
function main() {
    ...
    rawSamples.forEach((sample) => {
        ...
        if (true
            ...
            && sample.features.retEarly > 0.2
            // && sample.features.dollarVolEarly > 1000000
            // && sample.features.retEarly < 0.2
            // && sample.features.rangeEarly > 0.004
            // && sample.features.posInRange > 0.5
        ) {
    ...
    Object.keys(samplesByDay).sort((a, b) => {
        ...
        const topReturn = samplesToUse.length > 0 ? arrAve(samplesToUse.map((sample, i) => {
            ...
            let stopLoss = 0.95;    // MAY BE OVERWRITTEN!!!
            
            // fixed takeProfit
            if (exitMode === "fixed") {
                const takeProfit = 1 + (sample.features.retEarly / 1.5);
                // const takeProfit = 1 + (0.06);
                // const takeProfit = 1 + (sample.features.retEarly * (preHighIdx / 15));
                // const takeProfit = 1 + ((sample.features.retEarly / 2.5) * (latestPrice / prior1MinLow));
                // const takeProfit = 1 + ((sample.features.retEarly / 1.5) * (sample.features.posInRange) * (1 - (preHighIdx / 30)));
                // const stopLossBit = 0.15 * sample.features.posInRange;
                // stopLoss = 1 - stopLossBit;
                // const takeProfit = 1 + ((sample.features.retEarly / 1.5) * (preHighIdx / 25) * (rsiNow / rsi5Back));
                // const takeProfit = Math.max(1 + (preHighIdx / 400), 1.05);
                // const takeProfit = 1.15;

                // iterate for more accuracy
                const maxPrice = Math.max(...postBars.map(ele => ele.h));
                const maxPriceIdx = postBars.map(ele => ele.h).indexOf(maxPrice) + 1;
                postBars.forEach((postBar, i) => {

                    // TEMP - experiment on volRSI for exit strategy
                    // if (i > 16) {
                        const rsiLength = 14;
                        const postBarsSome = postBars.slice(0, i + 1);
                        const combineBars = preBars.concat(postBarsSome);
                        const rsiValsOld = combineBars.slice(combineBars.length - (rsiLength + 4), combineBars.length - 4).map(ele => ele.c);
                        const rsiValsNew = combineBars.slice(combineBars.length - (rsiLength), combineBars.length).map(ele => ele.c);
                        const rsiOld = calculateRSI(rsiValsOld);
                        const rsiNew = calculateRSI(rsiValsNew);
                        
                        const priceNow = postBar.c;
                        const returnRatio = priceNow / maxPrice;
                        scores.push(rsiNew / rsiOld);
                        results.push(returnRatio);
                        // if (!exitPrice && (rsiNew / rsiOld > 5)) {
                        //     exitPrice = postBar.c;
                    ...
                    // if (i > 10) {
                        ...
                        // const adjustedTakeProfitBit = (takeProfit - 1) * ((Math.max(i, 20) / postBars.length));
                        // const adjustedTakeProfit = 1 + adjustedTakeProfitBit;
                        const adjustedTakeProfit = takeProfit;
                        if (!exitPrice && postBar.h > adjustedTakeProfit * sample.entryPrice) {
                            numTakeProfitTrades += 1;
                            exitPrice = adjustedTakeProfit * sample.entryPrice;
                            if (i - 2 > maxPostIdx) {
                                tooLate += 1;
                            }
                            if (i + 2 < maxPostIdx) {
            ...
            const tradeRatio = exitPrice / sample.entryPrice;
            ...
            // scores.push(sample.features.dollarVolEarly);
            const totalRSI = calculateRSI(preBars.map(ele => ele.c));
            const volRSI = calculateRSI(preBars.slice(preBars.length - 14, preBars.length).map(ele => ele.v));
            // scores.push(sample.features.retEarly * volRSI);
            // results.push(tradeRatio);

            // if (sample.features.posInRange > 0.8) {
