...
function runDay(dateToRun) {
    ...
    for (let i = 3; i < 79; i += 1) {
        ...
        if (!own) {
            ...
            let score = 0;
            allSyms.forEach((sym) => {
                if (data[sym].length === 79 && data[sym][0].c > 5 && data[sym][i].v > 10000 && i < 78) {
                    ...
                    if (true
                        ...
                        // && volD > 5 * arrAve([volA, volB, volC])
                        && sampleD < 0.999 * sampleZero
                        // && sampleD > sampleC
                        // && data[sym][i].h < 1.01 * data[sym][i].l
                    ) {
