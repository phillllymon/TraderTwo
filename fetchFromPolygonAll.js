...
const { createSimpleDaysArr, dataArrToSymObj, arrAve, createFractionMap, sumDiffsForFractionMaps } = require("./util");
const fs = require("fs");
...
const diffAveAvesGood = [];
const diffAveAvesBad = [];
...
function runDayDataRecursive(data, i) {
    return new Promise((resolve) => {
        ...
        if (!dataToUse) {
            ...
            console.log("diff ave good: " + arrAve(diffAveAvesGood));
            console.log("diff ave bad: " + arrAve(diffAveAvesBad));
            console.log(goodMaps.length);
            console.log(badMaps.length);
            resolve();
        } else {
            fetchMinutelyDataForSymsRecursive(dataToUse.syms, dataToUse.date, 0).then((symDataObj) => {
                ...
                dataToUse.syms.forEach((sym, idx) => {
                    ...
                    if (goodMaps.length > 0 && badMaps.length > 0) {
                        const goods = [];
                        const bads = [];
                        goodMaps.forEach((goodMap) => {
                            const thisMap = createFractionMap(symData);
                            goods.push(sumDiffsForFractionMaps(thisMap, goodMap));
                        });
                        badMaps.forEach((badMap) => {
                            const thisMap = createFractionMap(symData);
                            bads.push(sumDiffsForFractionMaps(thisMap, badMap));
                        });
                        useSym = arrAve(goods) < 0.85 * arrAve(bads);
                    }
                    ...
                    if (tomorrowClose < tomorrowOpen) {
                        diffAveAvesGood.push(diffAve);
                    }
                    if (tomorrowClose > tomorrowOpen) {
                        diffAveAvesBad.push(diffAve);
                    }

                    // useSym = tomorrowClose < tomorrowOpen;
                    
                    // console.log("num: " + symData.length);
                    ...
                    if (tomorrowOpen > tomorrowClose) {
                        goodMaps.push(createFractionMap(symData));
                        if (goodMaps.length > 10) {
                            // goodMaps.shift();
                        }
                    }
                    if (tomorrowOpen < tomorrowClose) {
                        badMaps.push(createFractionMap(symData));
