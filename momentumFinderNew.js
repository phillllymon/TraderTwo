...
// test ---------------------
// fetchMinutelyOneDayOneSym("AAPL", todayStr).then((res) => {
//     console.log(res[res.length - 1]);
//     console.log("aplaca below");
//     fetchMinutelyOneDayOneSymAlpaca("AAPL", todayStr).then((r) => {
//         console.log(r[r.length - 1]);
//     });
// });
let alpacaMissing = 0;
let polygonMissing = 0;
let bothMissing = 0;
let alpacaSmaller = 0;
let polygonSmaller = 0;
let alpacaLengthZero = 0;
let polygonLengthZero = 0;
let noResults = 0;
main().then(() => {
    console.log("alpaca zero: " + alpacaLengthZero);
    console.log("polygon zero: " + polygonLengthZero);
    console.log("aplaca missing: " + alpacaMissing);
    console.log("polygon missing: " + polygonMissing);
    console.log("both missing: " + bothMissing);
    console.log("aplaca smaller: " + alpacaSmaller);
    console.log("polygon smaller: " + polygonSmaller);
    ...
    return new Promise((resolve) => {
        ...
        bothSetsOfData.then((results) => {
            if (results[0] && results[1]) {
                ...
                if (results[1].length > results[0].length) {
                    polygonSmaller += 1;
                }
            } else {
                if (!results[0] && !results[1]) {
